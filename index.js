...
app.get('/name', isAuthenticated, async function(req, res) {
  console.log("Getting with user id: " + tempId);
  let name = await dbo.getNameWithId(tempId).catch(error => {
  ...
  //   console.log(`Sending: ${result} for /name`);
...
app.post('/store', isAuthenticated, async function(request, response, next) {
  ...
  await dbo.post_activity(tempId,activity)
...
app.get('/reminder', isAuthenticated, async function(request, response, next) {
  ...
  let result = await dbo.get_most_recent_planned_activity_in_range(tempId, 0, currTime)
...
app.get('/week', isAuthenticated, async function(request, response, next) {
  ...
  if (activity === undefined) {
    let result = await dbo.get_most_recent_entry(tempId)
    try {
  ...
  let max = date
  let result = await dbo.get_similar_activities_in_range(tempId, activity, min, max)
...
//testing by checking db
dbo.getAllActivities().catch(
  function(error) {
    console.log("error:", error);
  }
)

dbo.getAllProfiles().catch(
  function(error) {
    console.log("error:", error);
  }
)

// dbo.deleteAllActivities().catch(
//   function(error) {
//     console.log("error:", error);
//   }
// )

// dbo.deleteAllProfiles().catch(
//   function(error) {
//     console.log("error:", error);
//   }
// )
...
async function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    let temp = profile.id.toString();
    temp = temp.substring(0,11); //converting to smaller number so that input properly
    temp = parseInt(temp);
    let val = await dbo.check_profile(temp, profile.name.givenName);
    if(val == undefined) {
      await dbo.add_profile(temp, profile.name.givenName);
      console.log(`Added: ${temp}, ${profile.name.givenName}`);
    }
    else{
      console.log(val);
      console.log(`Already have: ${temp}, ${profile.name.givenName}`)
    }
    let userid = temp;  
    tempId = temp;
    console.log("New temp ID: " + tempId);
    done(null, userid); 
}
...
passport.deserializeUser((userid, done) => {
    ...
    tempId = userid;
    let userData = {userData: "user data from db"}; //the way I did it works, but I could have put a database function call here 
    done(null, userData);
});
